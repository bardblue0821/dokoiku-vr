...
<x-app-layout>
    <div class="bg-white py-6 sm:py-8 lg:py-12">
        <div class="mx-auto max-w-screen-2xl px-4 md:px-8">
            ...
            <form method='post' action="{{route('tech_board.update', $tech_post)}}" enctype='multipart/form-data' class="mx-auto grid max-w-screen-md gap-8 sm:grid-cols-3">
                @csrf
                @method('patch')
                <div class="sm:col-span-3">
                    <p class="text-xl font-bold text-teal-500">タイトル <span class="text-base text-orange-400">(必須)</span></p>
                    <input name="title" id='title' value="{{ old('title', $tech_post->title) }}" placeholder="タイトル" class="w-full rounded border bg-gray-50 px-3 py-2 text-gray-800 outline-none ring-indigo-300 transition duration-100 focus:ring" />
